...
class ContinuousTaskChecker:
    ...
    def send_notification(self, title, message, priority="default", tags=None):
        ...
        try:
            ...
            # Only send the message body, title is already in the header
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.send_notification(
                    "🟢 Bot Online",
                    f"{current_ist.strftime('%I:%M %p IST')} | Checker active",
                    priority="default",
    ...
    def check_and_notify_tasks(self):
        ...
        if total_claimable > 0:
            notification_msg = f"🎯 {total_claimable}/{total_available} safe to claim | {current_time.strftime('%I:%M %p')}"
            priority = "high"
            tags = "bell"
            title = f"✅ {total_claimable} Safe Task{'s' if total_claimable != 1 else ''}"
        else:
            notification_msg = f"{total_available} available, 0 safe | {current_time.strftime('%I:%M %p')}"
            priority = "default"
            tags = "inbox_tray"
            title = "⚠️ No Safe Tasks"
    ...
    def run(self):
        ...
        try:
            ...
            self.send_notification(
                "🔴 Bot Stopped",
                f"{stop_time.strftime('%I:%M %p IST')} | {loop_count} checks done",
                priority="default",
                tags="stop_sign"
            )
